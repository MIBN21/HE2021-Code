...
govexpendplot<-data.frame(IncomeQuintile,Governmentspend_Pre,AmPretax,Governmentspend_Post,AmPosttax)
...
e<-ggplot(propplot,aes(x=reorder(propid,propvalue),y=propvalue,fill=propid))+
  geom_bar(stat="identity",position="dodge",width = 0.5)
e.labs <- e + labs(title = "Proportion of Population above Poverty Line", x = "Proportion", y = "Percentage (%)") + theme(plot.title = element_text(hjust = 0.5))+ scale_fill_discrete(name = "Pre/Post Policy",labels = c("PostPolicy", "PrePolicy"))
e.labs