...
#Part B and C
Governmentspend<-c(Government_Spending_Total,Government_Spending_Q1,Government_Spending_Q2,Government_Spending_Q3,Government_Spending_Q4,Government_Spending_Q5)
#Part D
...
IncomeQ5=sum(Income_Data_Total[(length(income_4)+1):length(income),1])
...
##Assume poverty line is 1000000yenb (Need do research)
AboveLinePre<-Income_Data[!Income_Data$Income<=1000000,]
AboveLinePost<-Income_Data_with_Redistributive[!Income_Data_with_Redistributive$Income<=1000000,]
...
colnames(Summary_statistics_Red) <- c("Income_Bracket","Old_Gini","Post_Policy_Gini","Government_Expenditure","Average_Benefit_of_Policy","Proportion_above_Poverty_Line(Pre)","Proportion_above_Poverty_Line(Post)")
